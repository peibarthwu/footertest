  ...
  mounted() {
   window.addEventListener("mousemove", (e) => {
     ...
     window.addEventListener("deviceorientation", (e) => {
      ...
      if(controls.enabled){
        controls.update();
        // this.mouse.x = Math.abs(Math.sin(controls.alpha)) * window.innerWidth;
        this.move(e);
      }
    });
  },
  methods: {
    ...
    move(e) {
      ...
      dashes.forEach((dash) => {
        ...
        var  dist = Math.sqrt(Math.pow(dashx-x,2) + Math.pow(dashy-y,2));
        if(controls.enabled){
          console.log(Math.abs(Math.sin(controls.alpha)) * window.innerWidth);
          dist = Math.sqrt(Math.pow(dashx-(Math.abs(Math.sin(controls.alpha)) * window.innerWidth),2) + Math.pow(dashy-this.mouse.y,2));
        }
